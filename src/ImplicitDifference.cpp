...
float Difference::FieldValue(const glm::vec3& point)
{
	...
	return std::min(m_left_child->FieldValue(point),  (2 * m_iso) - m_right_child->FieldValue(point));
}

glm::vec3 Difference::Normal(const glm::vec3& point)
{
	if (m_left_child->FieldValue(point) > (2 * m_iso) - m_right_child->FieldValue(point))
